...
class sfCaptchaGD {
  ...
  function __construct() {
    ...
    $this->fonts_dir = sfConfig::get('app_sf_captchagd_fonts_dir', sfConfig::get('sf_plugins_dir') . '/mastodontePlugin/data/fonts/');
  ...
  private function allocateColor($img, $color = "") {
    return imagecolorallocate($img, hexdec(substr($color, 0, 2)), hexdec(substr($color, 2, 2)), hexdec(substr($color, 4, 2))
    );
  }
